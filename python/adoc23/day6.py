...
def solution1(all_input: str) -> int:
    ...
    for pos in range(num_runs):
        result *= calculate_num_possibilities(
            struct_input["Time"][pos], distance=struct_input["Distance"][pos]
        )
    return result
...
def solution2(all_input: str) -> int:
    ...
    for line in all_input.splitlines():
        key, values = line.strip().split(":")
        struct_input[key] = int("".join(re_num.findall(values)))
    return calculate_num_possibilities(
        struct_input["Time"], distance=struct_input["Distance"]
    )
